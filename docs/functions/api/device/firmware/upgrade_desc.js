...
function jsonResponse(body, status = 200) {
  return new Response(JSON.stringify(body, null, 2), {
    ...
    headers: {
      ...
      "cache-control": status < 400 ? `public, max-age=${CACHE_SECONDS}` : "no-store",
    },
  });
}
...
export async function onRequestGet(context) {
  ...
  try {
    ...
    return jsonResponse({ error: String(err.message || err) }, err.status || 502);
  }
  ...
  const body = {
    ...
    fullversion,
    size: binAsset.size,
    // Dummy — GitHub's release API doesn't expose an MD5 for assets (only
    // the sha256 `digest` below), and hashing the ~300MB asset on every
    // request is not implemented. The key is present because unisrv's
    // parser requires it, but this value will never match the real file.
